...
export async function addInstruction(instruction: Omit<Instruction, 'id'>): Promise<number> {
try {
    const timestamp = getLocalTimestamp();
    const keysStr = Array.isArray(instruction.keys) ? instruction.keys.join('+') : instruction.keys;
...
export async function updateInstruction(instruction: Instruction): Promise<void> {
try {
    const timestamp = getLocalTimestamp();
    const keysStr = Array.isArray(instruction.keys) ? instruction.keys.join('+') : instruction.keys;
    ...
    [keysStr, instruction.functions, instruction.remarks || null, timestamp, instruction.id]
    );
    ElMessage.success({
    message: '更新指令成功',
    duration: 1000,
...
try {
    await (window as any).api.safeQuery(`DELETE FROM instructions WHERE id = ?`, [id]);
    ElMessage.success({
    message: '删除指令成功',
    duration: 1000,
    });
...
export async function batchImportInstructions(instructions: Instruction[]): Promise<void> {
try {
